public class GameEasing1DCom
{
    ...
    private float _from;
    private GameEasingType _easingType;
    private bool _isResetTime = true;
    private float _curValue;

    public GameEasing1DCom()
    {
        Clear();
    }
    ...
    public void SetEase(float duration, GameEasingType easingType)
    {
    ...
    public float Tick(float from, float to, float dt)
    {
        this.time += dt;
        if (this.to != to)
        {
        ...
        float easedValue = GameEasingFuncUtil.EaseByType(this._easingType, t);
